...
- All content lives in the `<script type="text/x-dc">` block at the bottom of `index.html`: `POSTS`, `FOCUS`, `BIO`, `JOBS`, `CREDENTIALS`, `SKILLS`, `SOCIALS`. Add new talks or articles to the top of `POSTS` (`type` is `Talk` or `Article`). A post body is either inline P/H/Q blocks (`body`) or, with `md: true`, Markdown in `posts/<id>.md`
- Markdown articles: put the source in `posts/<id>.md` without the title (the page header shows `title` and `dek` from `POSTS`), with images in `images/writing/` referenced by root-absolute path, and links to other posts as `/writing/<id>/`. `scripts/markdown.mjs` converts it (headings, lists, pipe tables, `>` callouts, figures, footnotes `[^n]`); keep to that subset. Optional per-post `image`/`imageAlt` sets the social preview image
- Article diagrams use the site theme (dark background, `#ffe600` accent, Inter and JetBrains Mono) at 1200×630 so they double as the social preview. Keep the source as `posts/<name>.diagram.html` (an inline SVG) and render a 2× PNG with headless Chrome, e.g. `"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --hide-scrollbars --window-size=2400,1260 --virtual-time-budget=8000 --screenshot=out.png file://$PWD/posts/<name>.diagram.html`. Also render a light-theme version (site light palette, olive `#6e5f00` accent instead of yellow text) as `<name>-light.png` next to it: the build then emits both and the article shows the one matching the site theme; the social preview uses the dark one. Give a changed image a new filename so social caches refresh
...
- `robots.txt` points to the sitemap and disallows the mini-apps; `_config.yml` excludes `scripts/`, `posts/` and `CLAUDE.md` from the published site
